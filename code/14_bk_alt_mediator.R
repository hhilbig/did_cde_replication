...
out <- get_estimates(est_df = est_df, X_list_use = X_in[[3L]],
                   Z_list_use = Z_in[[2L]], n_folds = folds) %>% 
  mutate(cov_label = X_in_labs[3L],
         n_folds = folds)

saveRDS(out, file = "data/15_output.rds")
out <- readRDS("data/15_output.rds")
...
cat5_table <- out_df_all |>
  ...
  rename(`Baseline Mediator` = base_cond, `ACDE-BC (s.e.)` = estimate_ACDE_BC,
         `ATT (s.e.)` = estimate_ATT, `n (ACDE)` = n_ACDE_BC, `n (ATT)` = n_ATT) |>
  select(`Baseline Mediator`, `ACDE-BC (s.e.)`, `n (ACDE)`, `ATT (s.e.)`,
         `n (ATT)`, everything())
cat5_table
...
cat5_table |>
  ...
  gtsave(filename = "figures/cat5_table.tex")
