...
Z_in <- list(c(Z_list, Z_sq_terms, Z_int_terms, XZ_int_terms),
...
sa_data |>
  ...
  geom_label_repel(data = cov_table |> filter(q95_abs_diff > 0.17),
                   aes(x = q95_abs_diff, y = 0, label = label), size = 2.5,
                   nudge_y = -1.25, seed = 1234) + 
  labs(x = expression(Gamma), y = "ACDE-PC (m = Neutral)") 
  

ggsave("figures/sensitivity_analysis.pdf", width = 7, height = 4, device=cairo_pdf)
