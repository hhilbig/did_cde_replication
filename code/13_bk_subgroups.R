...
##setwd("C:/Users/hanno/Dropbox/Harvard/Projects/DiD_DirectEffects/Replication_Data/15_Broockman_Kalla")
...
df <- data %>% 
  dplyr::select(hh_id, treat_ind, matches("therm"),
                matches('gender_norm'),
                matches("trans"),
...
## Implement the CDE stuff

## Need to change everything to be changes

df <- df %>% 
  mutate(gender_nonconformity_t1_diff = gender_nonconformity_t1  - gender_nonconformity_t0,
...
X_list <- c('gender_norm_sexchange_t0', 'gender_norm_moral_t0',
...
M_list <- c('therm_trans_t1', 'therm_trans_t2',
            'therm_trans_t3', 'therm_trans_t4')
M_list_proper <- c("Trans feeling therm. (t_1)",
...
M_list %>% lapply(function(x) df[, x] %>% is.na() %>% sum())
...
Y_list <- c('miami_trans_law_t1_avg_diff', 'miami_trans_law_t2_avg_diff',
            'miami_trans_law_t3_avg_diff', 'miami_trans_law_t4_avg_diff')
Y_list_proper = c('Trans law support (t_1 - t_0)',
...
Z_list <- c('gender_norm_sexchange_t1_diff', 'gender_norm_moral_t1_diff',
...
Z_list <- c('gender_norm_sexchange_t1', 'gender_norm_moral_t1',
...
measures <- rbind(data.frame(var = X_list, type = 'X'),
...
sum(complete.cases(df[, c('therm_trans_t2',
                          'treated',
...
est_df <- est_df[complete.cases(df[, c('therm_trans_t2',
                                       'treated',
                                       'miami_trans_law_t3_avg_diff',
                                       'base_med',
                                       'gender_nonconformity_t0', 
...
get_estimates <- function(est_df, 
  ...
  out_list <- lapply(M_list, function(m) {
    lapply(Y_list, function(y) {
      ...
      big_form <- as.formula(paste0(y, ' ~ treated + base_med + ', m, '+', 
                                    paste0(X_list_use , 
                                           collapse = '+'), '+', 
      ...
      y_xz_mod <- as.formula(paste0('~ treated + base_med + ', m, '+', 
                                    paste0(X_list_use , 
                                           collapse = '+'), '+', 
                                    paste0(Z_list_use, collapse = '+')))
      y_x_mod <- as.formula(paste0('~ treated + base_med +', 
                                   paste0(X_list_use, 
                                          collapse = '+')))
      
      ## Pscore formula
      
      f_m_mod <- as.formula(paste0(m, '~ treated + base_med + ', 
      ...
      f_m_gamma_mod <- as.formula(paste0(m, '~ treated + base_med + ', 
                                         paste0(X_list_use %>% setdiff('therm_trans_t0'), 
                                                collapse = '+')))
      y_x_gamma_mod <- as.formula(paste0('~ treated + base_med +', m, '+', 
                                   paste0(X_list_use, 
                                          collapse = '+')))
      ...
      se_dr_m2_gamma_ml <- out_dr_gamma_ml$estimates["treated_1_2", "std.error"]
            ...
            df_dr_ml <- data.frame(estimate = c(est_dr_marginal_ml, est_dr_m0_ml, est_dr_m1_ml,
                                       est_dr_m2_ml),
                          std.error = c(se_dr_marginal_ml, se_dr_m0_ml, se_dr_m1_ml,
                                        se_dr_m2_ml),
                          mediator = m,
                          covars = 'Yes', intermed_covars = 'Yes',
                          flexible = 'Yes',
                          method = "ACDE-BC",
                          base_cond = c("Marginal",
                                        "m = Cool",
                                        "m = Neutral",
                                        "m = Warm"),
      ...
      df_dr_gamma_ml <- data.frame(estimate = c(est_dr_marginal_gamma_ml, est_dr_m0_gamma_ml, est_dr_m1_gamma_ml,
                                             est_dr_m2_gamma_ml),
                                std.error = c(se_dr_marginal_gamma_ml, se_dr_m0_gamma_ml, se_dr_m1_gamma_ml,
                                              se_dr_m2_gamma_ml),
                                mediator = m,
                                covars = 'Yes', intermed_covars = 'Yes',
                                flexible = 'Yes',
      ...
      ## Add to table
        ...
        m2 <- felm(as.formula(paste0('y ~ treated + base_med+', 
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')), 
                                            collapse = '+'))),
                   data = est_df_temp)


        m2_0 <- felm(as.formula(paste0('y ~ treated+',
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')),
                                            collapse = '+'))),
                     data = est_df_temp,
                     subset = base_med == 0)
        m2_1 <- felm(as.formula(paste0('y ~ treated+',
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')),
                                            collapse = '+'))),
                     data = est_df_temp,
                     subset = base_med == 1)
        m2_2 <- felm(as.formula(paste0('y ~ treated+',
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')),
                                            collapse = '+'))),
                     data = est_df_temp,
                     subset = base_med == 2)
        ...
        out_table_2 <- mlist %>% lapply(broom::tidy, conf.int = T) %>%
          reduce(rbind) %>%
          ...
          dplyr::select(estimate, std.error, mediator, covars,
      ...
      if (FALSE) {
        ...
        m3 <- felm(as.formula(paste0('y ~ treated + base_med+', 
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')), 
                                            collapse = '+'),
                                     '+',
                                     paste0(c(Z_list_use %>% setdiff('therm_trans_t0')), 
                                            collapse = '+'))),
                   data = est_df_temp)
        m4 <- felm(as.formula(paste0('y ~ treated + base_med +' , m, ' + ', 
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')), 
                                            collapse = '+'),
                                     '+',
                                     paste0(c(Z_list_use %>% setdiff('therm_trans_t0')), 
        ...
        m3_0 <- felm(as.formula(paste0('y ~ treated +',
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')),
                                            collapse = '+'),
                                     '+',
                                     paste0(c(Z_list_use %>% setdiff('therm_trans_t0')),
                     ...
                     data = est_df_temp,
                     subset = base_med == 0)
        m3_1 <- felm(as.formula(paste0('y ~ treated +',
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')),
                                            collapse = '+'),
                                     '+',
                                     paste0(c(Z_list_use %>% setdiff('therm_trans_t0')),
                     ...
                     data = est_df_temp,
                     subset = base_med == 1)
        m3_2 <- felm(as.formula(paste0('y ~ treated +',
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')),
                                            collapse = '+'),
                                     '+',
                                     paste0(c(Z_list_use %>% setdiff('therm_trans_t0')),
                     ...
                     data = est_df_temp,
                     subset = base_med == 1)
        m4_0 <- felm(as.formula(paste0('y ~ treated +', m, ' + ',
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')),
                                            collapse = '+'),
                                     '+',
                                     paste0(c(Z_list_use %>% setdiff('therm_trans_t0')),
                     ...
                     data = est_df_temp,
                     subset = base_med == 0)
        m4_1 <- felm(as.formula(paste0('y ~ treated +', m, ' + ',
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')),
                                            collapse = '+'),
                                     '+',
                                     paste0(c(Z_list_use %>% setdiff('therm_trans_t0')),
                     ...
                     data = est_df_temp,
                     subset = base_med == 1)
        m4_2 <- felm(as.formula(paste0('y ~ treated +', m, ' + ',
                                     paste0(c(X_list_use %>% setdiff('therm_trans_t0')),
                                            collapse = '+'),
                                     '+',
                                     paste0(c(Z_list_use %>% setdiff('therm_trans_t0')),
        ...
        out_table_3 <- mlist %>% lapply(broom::tidy, conf.int = T) %>%
          reduce(rbind) %>%
          filter(str_detect(term, 'treated')) %>%
          mutate(mediator = m,
                 covars = 'Yes',
        ...
        out_table_4 <- mlist %>% lapply(broom::tidy, conf.int = T) %>%
          reduce(rbind) %>%
          filter(str_detect(term, 'treated')) %>%
          mutate(mediator = m,
                 covars = 'Yes',
        ...
        df_dr <- data.frame(estimate = c(est_dr_marginal, est_dr_m0, est_dr_m1,
                                         est_dr_m2),
                            std.error = c(se_dr_marginal, se_dr_m0, se_dr_m1,
                                          se_dr_m2),
                            mediator = m,
                            covars = 'Yes', intermed_covars = 'Yes',
                            flexible = 'No',
        ...
        df_dr_gamma <- data.frame(estimate = c(est_dr_marginal_gamma, est_dr_m0_gamma, est_dr_m1_gamma,
                                               est_dr_m2_gamma),
                                  std.error = c(se_dr_marginal_gamma, se_dr_m0_gamma, se_dr_m1_gamma,
                                                se_dr_m2_gamma),
                                  mediator = m,
                                  covars = 'Yes', intermed_covars = 'Yes',
                                  flexible = 'No',
  ...
  out_df <- out_list %>%
    reduce(rbind) %>% 
    mutate(method = factor(method, levels = c('DiD w/ X, no mediator',
...
X_in <- list(c(X_list, X_sq_terms, X_int_terms),
             X_list,
             c('vf_democrat', 'ideology_t0', 
               ...
               'therm_obama_t0'),
             c("vf_female", "vf_hispanic", 
               "vf_black", "vf_age"))
X_in_labs <- c('1: All covars + sq terms + ints', 
               '2: All covars', 
               '3: Indices + Demographics',
               '4: Only Demographics in X')

Z_in <- list(Z_list, 
             c("therm_obama_t1_diff", 
               "trans.tolerance.dv.t1_diff",
               "gender_nonconformity_t1_diff",
               "miami_trans_law_t1_avg_diff"))
...
M_list_proper <- c("Trans feeling therm. (t_2)",
                   "Trans feeling therm. (t_3)")
Y_list <- c('miami_trans_law_t3_avg_diff', 'miami_trans_law_t4_avg_diff')
Y_list_proper = c('Trans law support (t_3 - t_0)',
                  'Trans law support (t_4 - t_0)')
...
out <- lapply(folds_in, function(folds) {
  lapply(3, function(X_id) {
      lapply(seq_along(subs), function(sub_g) {
        cat(subs_labs[sub_g], " n = ", sum(subs[[sub_g]]), '\n')

        if (sub_g %in% 2:3) {
          this_X <- setdiff(X_in[[X_id]], c('vf_hispanic', 'vf_black'))  
        } else if (sub_g %in% 4:5) {
          this_X <- setdiff(X_in[[X_id]], c('vf_female'))
        } else {
          this_X <- X_in[[X_id]]
        }
        o <- get_estimates(est_df = est_df, X_list_use = this_X,
                         Z_list_use = Z_in[[2L]], n_folds = folds, subset = subs[[sub_g]]) %>% 
          mutate(cov_label = X_in_labs[X_id],
                 n_folds = folds,
...
## saveRDS(out, file = "data/14_output.rds")
out <- readRDS("data/14_output.rds")

## Save this for plotting

out_df_all <- out %>% 
  left_join(data.frame(mediator = M_list,
                       mediator_lab = M_list_proper)) %>% 
  left_join(data.frame(outcome = Y_list,
                       outcome_lab = Y_list_proper)) %>% 
  mutate(p_med = as.numeric(str_extract(mediator, '[0-9]')),
         p_y = as.numeric(str_extract(outcome, '[0-9]'))) %>% 
  filter(!p_med > p_y) %>% 
  mutate(outcome_lab = haschaR::add_linebreak_vector(outcome_lab, min_length = 8),
  ...
  mutate(
    ...
    method = factor(method, levels = c('DID w/ X, no mediator',
                                            'DID w/ X, Z + mediator',
                                            "ACDE-BC",
                                            "ACDE-PC")),
    base_cond = factor(base_cond, levels = c('Marginal',
                                            'm = Cool',
                                            'm = Neutral',
                                            'm = Warm'))
  ) 
...
subgroup_table <- out_df_all |>
  ...
  rename(Subgroup = subgroup, `Baseline Mediator` = base_cond, `ACDE-BC (s.e.)` = estimate_ACDE_BC,
...
subgroup_table |>  
  ...
  gtsave(filename = "figures/subgroup_effects.tex")
  



time_effects <- out |>
  filter(method %in% c("ACDE-BC"),
         flexible == "Yes",
         base_cond == "m = Neutral",
         subgroup == "All") |>
  mutate(
  ...
  rename(`Outcome` = outcome, `Mediator` = mediator,
         `Estimate` = estimate, `Std. Error` = std.error)
time_effects

time_effects |>
  gt() |>
  gtsave(filename = "figures/overtime_effects.tex")
